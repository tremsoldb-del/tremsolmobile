...
class CurrencyConverterSelection extends StatefulWidget {
  const CurrencyConverterSelection({super.key});

  @override
  _CurrencyConverterSelectionState createState() =>
      _CurrencyConverterSelectionState();
}

class _CurrencyConverterSelectionState
    extends State<CurrencyConverterSelection> {
  ...
  // Common currency → ISO 3166-1 alpha-2 (for reliable flag CDN fallback)
  ...
  CountryInfo _normalizeCurrencyInfo(CountryInfo info) {
    if (info.currencyCode == 'USD') {
      // Always show USD as "United States" with US flag
      return CountryInfo(
        currencyCode: 'USD',
  ...
  Future<void> initData() async {
    ...
    await fetchApiKey();
    await fetchBaseCurrency(); // also triggers fetchAvailableCurrencies()

    // Load cached country/currency list (instant UI)
    ...
    // Refresh from network (no throws, has timeouts + fallbacks)
  ...
  Future<void> fetchApiKey() async {
    try {
      ...
      if (doc.exists && mounted) {
        ...
        debugPrint("API Key document not found");
      }
    } catch (e) {
  ...
  /// Fetch baseCurrency from Firestore and also cache it locally
  Future<void> fetchBaseCurrency() async {
    try {
      ...
      if (doc.exists) {
        ...
        // Cache for other screens (e.g. loadCurrencyData)
        final prefs = await SharedPreferences.getInstance();
        await prefs.setString('baseCurrency', value);
      } else {
        debugPrint("Base currency doc not found");
      }
    ...
    if (cachedData != null) {
      try {
        ...
        if (rates is Map && mounted) {
          ...
          // Debug check for USD presence
          debugPrint(
            'Has USD in availableCurrencies: '
    ...
    try {
      ...
      for (final pair in codes) {
        if (pair is List && pair.length >= 2) {
          ...
          final currencyName = pair[1].toString(); // fallback display
  ...
  Future<void> _selectAndProceed(String currency) async {
    ...
    if (rate != null) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            "Failed to fetch conversion rate. Please check your connection and try again.",
          ),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        automaticallyImplyLeading: true, // default back arrow
        title: const Text(
          "Choose currency",
        ...
        actions: [
          TextButton(
            onPressed: () async {
              ...
              // If user skips, treat baseCurrency as selected currency (rate 1.0)
              await prefs.setString('selectedCurrency', defaultCurrency);
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      // Hide the base/default currency from the list
