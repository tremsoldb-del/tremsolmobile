...
class BulkBuyPage extends StatefulWidget {
  const BulkBuyPage({super.key});

  @override
  State<BulkBuyPage> createState() => _BulkBuyPageState();
}

class _BulkBuyPageState extends State<BulkBuyPage> {
...
Future<void> _loadCurrencyData() async {
  ...
  Future<void> printOrderCount(String productId) async {
    // Get the order count for the given productId
    final orderCount = await getOrderCount(productId);

    // Print the result to the console
    print('Product $productId has been ordered $orderCount times.');
  }
...
//25-12-24
  String _capitalizeFirstLetter(String input) {
    if (input.isEmpty) {
      return input; // Return the input as it is if it's empty
    }
    return input
        .split(' ')
        .map((word) =>
 ...
 /* String _capitalizeFirstLetter(String input) {
  ...
  Widget build(BuildContext context) {
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(
                  builder: (context) => const PrdSearchDealsPage(dealTag: 'Bulk'),
                ),
              );
            },
          ),
    ...
    : StreamBuilder<QuerySnapshot>(
        ...
        backgroundColor: AppColors().getColor('bulk'),
