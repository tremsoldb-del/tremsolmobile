...
class PromoDealsPage extends StatefulWidget {
  const PromoDealsPage({super.key});
...
class _PromoDealsPageState extends State<PromoDealsPage> {
  ...
  final Duration _timeRemaining = const Duration(hours: 1); // unused but kept
  ...
  void initState() {
    super.initState();
    _loadCurrencyData();
    _loadPromoBanner(); // load banner + title from Firestore
  }
...
Future<void> _loadCurrencyData() async {
  ...
  /// 🔹 Read banner image + title from:
  /// collection: promo_banner
  /// document:  banner_id
  /// fields:    banner_value (image URL), banner_title (string)
  Future<void> _loadPromoBanner() async {
    try {
      ...
      if (doc.exists) {
        final data = doc.data();
        setState(() {
          bannerImageUrl = data?['banner_value'] as String?;
          bannerTitle = data?['banner_title'] as String?;
        });
      } else {
        debugPrint('promo_banner/banner_id document not found');
      }
    } catch (e) {
      debugPrint('Error loading promo banner: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    final titleText = bannerTitle ?? 'Promotions';
    ...
    return Scaffold(
          ...
          IconButton(
            ...
            onPressed: () {
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (context) =>
                      const PrdSearchDealsPage(dealTag: 'Promo'),
                ),
              );
            },
          ),
      ...
      body: Column(
        children: [
          // 🔹 Top banner like in your screenshot
          if (bannerImageUrl != null && bannerImageUrl!.isNotEmpty)
            Padding(
              padding: const EdgeInsets.only(bottom: 1.0),
              child: AspectRatio(
                aspectRatio: 16 / 6, // adjust as you like
                child: ClipRRect(
        ...
        return Padding(
          ...
          child: GridView.builder(
            ...
            itemBuilder: (context, index) {
              // ... your existing itemBuilder code unchanged ...
              // (keep everything you already had here)
              // I’m not repeating it to keep this reply shorter.
              
              // --- START of original card code ---
              ...
              final discountedPercentage =
                  (((iniSellingprice - finSellingprice) /
                          (iniSellingprice == 0 ? 1 : iniSellingprice)) *
                      100);
                              ...
                              _placeOrder(
                                {
                                  'id': product.id,
                                  'productname': name,
                                  'productsellingprice': finSellingprice,
